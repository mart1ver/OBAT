...
<?php 
    ...
    $req->execute(array('id_chantier' => $_GET["id"]));
           ...
           ?>
                ...
                <option value="recherche.php?id=<?php echo $_GET["id"]?>&fid=<?php echo($donnees['id'])?>&rcorps=<?php echo($_GET["rcorps"]);?>&rtypo=<?php echo($_GET["rtypo"]);?>&ro=<?php echo($_GET["ro"]);?>&rm=<?php echo($_GET["rm"]);?>" <?php if($donnees['id'] == $_GET["fid"] ){echo'selected="selected"';} ?>><?php echo($donnees['nom']);?></option>
